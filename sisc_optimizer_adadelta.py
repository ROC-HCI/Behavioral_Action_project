...
def shrink(alpha, threshold):
    N,D = np.shape(alpha)
    assert(N>D)
    alpha= np.sign(alpha)*np.maximum(np.zeros_like(alpha),np.abs(alpha)-threshold)
...
def optimize_proxim(X,M,D,beta,iter_thresh=65536,\
    ...
    while iter < iter_thresh:
        ...
        # Update psi with RMSProp
        # ========================
        # Calculate gradient
        grpsi = calcGrad_psi(alpha,psi,X,workers)
        # Accumulate gradient
        cacheg_psi=rho*cacheg_psi + (1.0-rho)*(grpsi**2.0)
        rmsg = np.sqrt(cacheg_psi + eps)
                        
        # Update
        gamma_psi = (1.0/rmsg)
        dpsi = -gamma_psi*grpsi
        psi = projectPsi(psi + dpsi,1.0)
            
        # Update alpha with RMSProp
        # ===========================
        gralpha = calcGrad_alpha(alpha,psi,X,workers)
        # Accumulate gradient
        cacheg_alpha=rho*cacheg_alpha + (1.0-rho)*(gralpha**2.0)
        rmsg = np.sqrt(cacheg_alpha + eps)
                        
        # Update
        gamma_alpha = (1.0/rmsg)
        dalpha = -gamma_alpha*gralpha
        alpha = shrink(alpha + dalpha,gamma_alpha*beta/N)
    ...
    # Final report on the optimization
